...
export const compressImageIfNeeded = async (file, options = {}) => {
  ...
  if (file.size <= maxSizeBytes) {
    console.log('Image is already under size limit, no compression needed');
    return file;
  }
  ...
  try {
    ...
    console.log(`Compression complete: ${originalSize}MB → ${compressedSize}MB (${compressionRatio}% reduction)`);
 ...
 */
export const compressImageWithQuality = async (file, quality = 0.8, maxSizeMB = 5) => {
  const options = {
    ...defaultOptions,
    quality,
    maxSizeMB,
...
export const validateImageFile = (file, maxSizeMB = 5) => {
  ...
  if (file.size > maxSizeBytes) {
    return { 
      success: false, 
      message: `File too large. Maximum size is ${maxSizeMB}MB. Current size: ${formatFileSize(file.size)}` 
    };
  }
